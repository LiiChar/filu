...
function rgbToHsl(r: number, g: number, b: number) {
	...
	return [h * 360, s, l]; // h,s,l: h[0..360], s,l[0..1]
}

export function rgbToHex(r: number, g: number, b: number): string {
	const toHex = (value: number) => {
		const clamped = Math.max(0, Math.min(255, value)); // ограничиваем 0–255
...
export const setColorPrimary = (url: string): string => {
	...
	image.onload = () => {
		try {
			const colorRGB = color.getColor(image);

			let [h, s, l] = rgbToHsl(colorRGB[0], colorRGB[1], colorRGB[2]);

			// гарантируем, что цвет не слишком тёмный/блеклый
			if (l < 0.35) l = 0.35;
			if (l > 0.75) l = 0.75;
			if (s < 0.5) s = 0.5;
	...
	image.onerror = (e) => {
		console.error('Не удалось загрузить изображение по причине: ', e);
	};
